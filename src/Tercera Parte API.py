...
async def fetch_items_by_search_term(
    client: httpx.AsyncClient, term: str, limit: int = 50
) -> list:
...
async def fetch_item_details(client: httpx.AsyncClient, item_id: str) -> dict:
...
def save_items_to_csv(items, output_file: str) -> None:
    fieldnames = [
        "id",
        "title",
        "category_id",
